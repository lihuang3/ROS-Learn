...
//    rosparam set /publish_velocity/max_vel 2.0;
    // Get the maximum velocity parameter .
    ros::param::set("/turtlesim/max_vel", 0.1);

    const std::string PARAM_NAME = "/turtlesim/max_vel";
    double maxVel;
...
//        }
        ...
        // control law
        if( fabsf(angErr) > 1.0){
            msg1.linear.x = fmin(0.2,maxVel); // if angular error is large, turn (mostly) in place
            msg2.linear.x = fmin(0.2,maxVel);
         }else{
            msg1.linear.x = fmin(distErr, maxVel);
            msg2.linear.x = fmin(distErr, maxVel);
         }
