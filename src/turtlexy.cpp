...
int main(int argc, char **argv) {
  ...
  while(ros::ok()) {
    ...
    Euclidean_distance = sqrt( pow(goal[0]-turtlePose.x,2) + pow(goal[1]-turtlePose.y,2));
    // Compute the goal orientation
    target_orient = atan2(goal[1]-turtlePose.y, goal[0]-turtlePose.x);
    ...
    if( ros::Time::now().toSec()-time>2.5){
        ...
        if( Euclidean_distance < 0.2){
            ...
            msg.angular.z = std::max(float(-1.0),std::min(float(1.0), target_orient-turtlePose.theta));
